...
STAMP="$(date +%Y%m%d-%H%M%S)-text-smoke"
...
usage() {
  cat <<'EOF'
Usage:
  bash ops/sunny/run-remote-text-smoke.sh [options]
...
ssh \
  ...
  "$SUNNY_SSH_TARGET" \
  "cd '$REMOTE_REPO_DIR' && bash ops/sunny/text-smoke.sh$REMOTE_ARGS_STR" \
...
if [[ "$RSYNC_EXIT_CODE" -eq 0 ]]; then
  if [[ -f "$LOCAL_REPORT_DIR/summary.json" ]]; then
    ...
    echo "run-remote-text-smoke.sh: expected summary.json under $LOCAL_REPORT_DIR after rsync" >&2
    SUMMARY_EXIT=2
  fi
else
  echo "run-remote-text-smoke.sh: rsync failed while copying $REMOTE_REPORT_DIR to $LOCAL_REPORT_DIR" >&2
  SUMMARY_EXIT=2
fi
