...
REPORT_STAMP="$(date +%Y%m%d-%H%M%S)-text-smoke"
REPORT_DIR="$ROOT_DIR/artifacts/sunny-reports/$REPORT_STAMP"
DATASET_ROOT="$ROOT_DIR/data/text"
EPOCHS=1
DEVICE="cuda"
...
usage() {
  cat <<'EOF'
Usage:
  bash ops/sunny/text-smoke.sh [options]

Options:
  --report-dir PATH       Report directory on Sunny
  --output-dir PATH       Output directory for training artifacts
  --dataset-root PATH     Dataset cache root for text training
  --epochs N              Number of training epochs (default: 1)
  --device NAME           Training device (default: cuda)
  --model-name NAME       Hugging Face model name (default: distilbert/distilbert-base-uncased)
  --train-sample-limit N  Training sample limit (default: 512)
  --val-sample-limit N    Validation sample limit (default: 128)
...
if [[ "$OUTPUT_DIR_SET" == false ]]; then
  OUTPUT_DIR="$REPORT_DIR/text-artifacts"
fi
...
capture_cmd \
  "windows-text-smoke-console" \
  "$POWERSHELL" \
  -NoProfile \
  -ExecutionPolicy Bypass \
  -File "$(wslpath -w "$ROOT_DIR/ops/sunny/text-smoke.ps1")" \
  ...
  -TrainSampleLimit "$TRAIN_SAMPLE_LIMIT" \
  -ValSampleLimit "$VAL_SAMPLE_LIMIT"

capture_cmd \
  "windows-text-summary-check" \
  python3 "$ROOT_DIR/ops/sunny/exit_from_summary_json.py" "$REPORT_DIR/windows-text-smoke-summary.json"
