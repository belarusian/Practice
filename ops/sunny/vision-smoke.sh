...
REPORT_STAMP="$(date +%Y%m%d-%H%M%S)-vision-smoke"
REPORT_DIR="$ROOT_DIR/artifacts/sunny-reports/$REPORT_STAMP"
DATASET_ROOT="$ROOT_DIR/data/vision"
EPOCHS=1
DEVICE="cuda"
RESTORE_DEMO=true
...
if [[ "$OUTPUT_DIR_SET" == false ]]; then
  OUTPUT_DIR="$REPORT_DIR/vision-artifacts"
fi
...
capture_cmd \
  "windows-vision-smoke-console" \
  "$POWERSHELL" \
  -NoProfile \
  -ExecutionPolicy Bypass \
  -File "$(wslpath -w "$ROOT_DIR/ops/sunny/vision-smoke.ps1")" \
