...
usage() {
  cat <<'EOF'
Usage:
  bash ops/sunny/run-remote-training-proof.sh [options]

Options:
  --target NAME          Training target to probe remotely (default: vision)
  --device NAME          Device to probe remotely (default: cuda)
...
if [[ "$WITH_TRAINING_MODE" == true ]]; then
  REMOTE_ARGS+=("--with-training-mode")
fi

if [[ "$RESTORE_DEMO" == true ]]; then
  REMOTE_ARGS+=("--restore-demo")
fi
...
ssh \
  ...
  "$SUNNY_SSH_TARGET" \
  "cd '$REMOTE_REPO_DIR' && bash ops/sunny/prove-training-runtime.sh$REMOTE_ARGS_STR" \
...
if [[ "$RSYNC_EXIT_CODE" -eq 0 ]]; then
  if [[ -f "$LOCAL_REPORT_DIR/summary.json" ]]; then
    ...
    echo "run-remote-training-proof.sh: expected summary.json under $LOCAL_REPORT_DIR after rsync" >&2
    SUMMARY_EXIT=2
  fi
else
  echo "run-remote-training-proof.sh: rsync failed while copying $REMOTE_REPORT_DIR to $LOCAL_REPORT_DIR" >&2
  SUMMARY_EXIT=2
fi
