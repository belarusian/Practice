"""GPT-2 model architecture implementation (NumPy-only).
...
from .tensor import Tensor, matmul, relu, gelu, softmax, zeros, ones, randn, random_init
...
class Attention:
    ...
    def __init__(
        ...
        self.bias = np.tril(np.ones((block_size, block_size)))
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, GPT2Cache]:
        ...
        q = self.c_q.forward(x)
        k = self.c_k.forward(x)
        v = self.c_v.forward(x)
        ...
        v = v.data.transpose(0, 2, 1, 3)
        
        k_t = k.transpose(0, 1, 3, 2)
        scores = matmul(Tensor(q), Tensor(k_t)).data / np.sqrt(self.head_dim)
        scores = scores - 1e9 * (1 - self.bias[:seq, :seq])
        
        attn_weights = softmax(Tensor(scores), axis=-1).data
        
        out = matmul(Tensor(attn_weights), Tensor(v)).data
...
class GPT2:
    """GPT-2 style transformer language model (NumPy-only).
