...
class BPETokenizer:
    ...
    def __init__(
        ...
        # Merge rules: (left, right) -> new_id
        self.merge_rules: Dict[Tuple[int, int], int] = merge_rules or {}
        
        # Reverse merge rules for decoding
        ...
        # Reverse vocab for fast lookup
        self.reverse_vocab: Dict[bytes, int] = {v: k for k, v in self.vocab.items()}
        
        # Special tokens
        self.bos_token: int = self.vocab_size - 2 if vocab is None else self._get_token_id("<|endoftext|>")
    ...
    def _pre_tokenize(self, text: str) -> List[str]:
        ...
        words = self._GPT2_PATTERN.findall(text)
        return words
    ...
    def _init_vocab_from_freq(self, freq: Dict[str, int]) -> Dict[int, bytes]:
        ...
        # Add all unique bytes from words
        for word in freq:
    ...
    def _learn_merge_rules(
        ...
        # Convert words to list of token IDs
        ...
        # Initialize word tokenizations
        word_tokens = {}
        for word in freq:
            word_tokens[word] = [vocab[b] for b in word.encode("utf-8")]
        
        # Start merge ID after base vocabulary
        merge_id = 258  # After base bytes and special tokens
        
        merge_rules = {}
        
        # Learn merge rules until vocab is full
        while len(vocab) < self.vocab_size:
            # Find most frequent pair across all words
            ...
            # Check if pair exists in vocab
            if pair in self.reverse_vocab:
                continue
            
            # Create new merge rule
            ...
            if verbose and merge_id % 1000 == 0:
                print(f"  Merge {merge_id}: {pair} -> {merge_id} (freq: {pair_count})")
    ...
    def decode(self, tokens: List[int]) -> str:
        ...
        for token in tokens:
            if token in self.vocab:
                ...
                # Unknown token - skip or handle specially
                pass
